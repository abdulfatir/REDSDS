...
def plot_results(result, prefix=""):
    ...
    discrete_states_lk = torch.exp(result["log_gamma"][0])[0].data.cpu().numpy()
    true_seg = None
    if "true_seg" in result.keys():
        true_seg = result["true_seg"][0, : config["context_length"]].data.cpu().numpy()
...
if __name__ == "__main__":
    ...
    for step in range(start_step, config["num_steps"] + 1):
        try:
            ...
            train_gen = iter(train_loader)
        train_result = train_step(train_batch, model, optimizer, step, config, device)
